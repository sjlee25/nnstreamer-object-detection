...
class ObjectDetection:
    ...
    def __init__(self, argv=None):
        ...
        parser.add_argument('--images', type=str, help='image set path')
    ...
    def run(self):
        ...
        os.makedirs(f'{self.images}_1')

        for image in os.listdir(self.images):
            self.image_names.append('.'.join(image.split('.')[:-1]))
            os.rename(f'{self.images}/{image}', f'{self.images}_1/{i}.jpg')
            i += 1

        # gstreamer pipeline
        pipeline = f'multifilesrc location="{self.images}_1/%d.jpg" ! decodebin name=decode decode. ! videoscale '

        pipeline += f'''
            ! videoconvert ! video/x-raw,width={self.video_width},height={self.video_height},format=RGB ! tee name=t  
            t. ! queue ! videoconvert ! cairooverlay name=tensor_res tensor_res. ! ximagesink 
        ...
        # frame count
        self.frame_tensor_sink = self.pipeline.get_by_name('tensor_sink')
        self.frame = 0
        ...
        # make out file
        folder_path = './output/' + self.images.split('/')[-1]
        ...
        for image_name in self.image_names:
            os.rename(f'{self.images}_1/{i}.jpg', f'{self.images}/{image_name}.jpg')
            i+=1
        os.rmdir(f'{self.images}_1')
    ...
    def on_new_data(self, sink, buffer):
        ...
        if self.running and buffer.n_memory() == 4:
            ...
            success, self.frame = self.frame_tensor_sink.query_position(Gst.Format.DEFAULT)

            self.get_detected_objects(num_detections, detection_classes, detection_scores, detection_boxes, self.image_names[self.frame])
